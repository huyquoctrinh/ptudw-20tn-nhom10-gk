...
controller.showDraft = async (req, res) => {
    ...
    rows.forEach((article) => {
        ...
        let day = d + '/' + m + '/' + y;
        article.createDay = day;
        if (article.ArticleStatuses.length != 0 && article.ArticleStatuses[0].status == 'Draft'){
...
controller.showApprove = async (req, res) => {
    ...
    rows.forEach((article) => {
        ...
        let day = d + '/' + m + '/' + y;
        article.createDay = day;
        if (article.ArticleStatuses.length == 0 || article.ArticleStatuses[0].status == 'Published'){
            article.status = 'Published';
            posts.push(article);
        }
        article.briefDescription = article.briefDescription.substring(0, 100);
...
controller.showReject = async (req, res) => {
    ...
    rows.forEach((article) => {
        ...
        let day = d + '/' + m + '/' + y;
        article.createDay = day;
        if (article.ArticleStatuses.length != 0 && article.ArticleStatuses[0].status == 'Rejected'){
